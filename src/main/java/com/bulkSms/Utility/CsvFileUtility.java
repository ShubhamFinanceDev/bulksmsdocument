...
public class CsvFileUtility {
    ...
    public List<Object> csvBulksms(InputStream inputStream) throws Exception {
        List<DataUpload> dataUploadList = new ArrayList<>();
        List<Object> ultimateResponse = new ArrayList<>();
        List<Object> responsiveList = new ArrayList<>();
        ...
        try (BufferedReader bReader = new BufferedReader(new InputStreamReader(inputStream, "UTF-8"));
            ...
            for (CSVRecord record : csvRecords) {
                String mobileNumber = record.get(1);
                if (mobileNumber.matches("\\d{10}")) {
                    DataUpload dataUpload = new DataUpload();
                    dataUpload.setLoanNumber(record.get(0));
                    dataUpload.setMobileNumber(mobileNumber);
                    dataUpload.setCertificateCategory(record.get(2));
                    dataUpload.setSmsFlag(record.get(3));
                    dataUploadList.add(dataUpload);
                } else {
                    Map<String, Object> response = new HashMap<>();
                    response.put("mobileNo", mobileNumber);
                    responsiveList.add(response);
                }
            }
            dataUploadRepo.saveAll(dataUploadList);
            Map<String, Object> response1 = new HashMap<>();
            response1.put("msg", "csv uploaded successfully");
            response1.put("invalid mobileNumber list", responsiveList);
            ultimateResponse.add(response1);
